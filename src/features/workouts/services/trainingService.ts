...
import type {
    AvailableTrainingSlotsParams,
    AvailableTrainingSlotsResponse,
    TrainingResponse,
    TrainingRequest,
...
export const getAvailableTrainingSlots = (
    params: AvailableTrainingSlotsParams,
): Promise<AvailableTrainingSlotsResponse> => {
    const search = new URLSearchParams({
        trainerUsername: params.trainerUsername,
        traineeUsername: params.traineeUsername,
        date: params.date,
    })
    if (params.excludeTrainingId != null) {
        search.set('excludeTrainingId', String(params.excludeTrainingId))
    }
    return request<AvailableTrainingSlotsResponse>(
        `/trainings/available-slots?${search.toString()}`,
